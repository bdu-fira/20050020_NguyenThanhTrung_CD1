...
namespace Models.DAO
{
    ...
    public class ProductDao
    {
        ...
        public List<Product> CateKorea()
        {
            ...
            return db.Products.Where(n => n.CateId == 7).Take(4).ToList();
        }
        public List<Product> CateHavana()
        {
            //var date = DateTime.Now.Date;
            return db.Products.Where(n => n.CateId == 8).Take(4).ToList();
        }

        // Phương thức GetProductByCategory mới
        public List<Product> GetProductByCategory(int cateId, int count = 4)
        {
            var date = DateTime.Now.Date;
            return db.Products
                     .Where(n => n.Discount == 0 || n.EndDate < date || n.StartDate > date)
                     .Where(n => n.CateId == cateId)
                     .Take(count)
                     .ToList();
        }
